...
**Architecture** : 8 modules (248h total), JavaScript pur (pas TypeScript), 6 dependencies (1.6 MB), multi-OS natif.
...
**Phase 5 - Troubleshooter (40h, jours 22-26)**:
- Diagnostic erreurs communes :
  - Node version < 18 → Suggest upgrade, link docs
  - Permissions denied → Suggest chmod/chown, link docs
  - Paths invalides → Detect OS, suggest fix
...
**Total Effort**: 248h across 8 phases (6 weeks, 2 devs)

**Phases**:
1. Detection (40h, jours 3-7) - 8 fichiers ← **PHASE 1 DETAILED BELOW**
2. Recommender (24h, jours 8-10) - 1 module
3. Installer (56h, jours 11-17) - 1 module + templates ← **MOST COMPLEX**
4. Validator (32h, jours 18-21) - 1 module + 10 checks
5. Troubleshooter (40h, jours 22-26) - 1 module + diagnostics
6. Backuper (24h, jours 27-29) - 1 module + cleanup
7. Interviewer (16h, jours 30-31) - 1 module + questions
8. Wizard (16h, jours 32-33) - 1 module + post-install flow
...
**Phase 3: Installer (56h)** ← **MOST COMPLEX**

- [ ] **Story 3.1**: Implement 3 installation modes (Full/Minimal/Custom) (16h)
...
- [ ] **Story 3.2**: Implement multi-platform installation (Copilot CLI stubs) (12h)
  - Action: Generate `.github/agents/*.md` stubs with YAML frontmatter
  - Tests: 8 tests (generate stub, frontmatter valid, multiple agents, overwrite detection)
...
- [ ] **Story 3.4**: Implement multi-platform installation (Claude Code MCP) (12h)
  - Action: Update `claude_desktop_config.json` with MCP servers section
  - Tests: 8 tests (OS-specific paths, JSON merge, existing config, backup)

- [ ] **Story 3.5**: Implement multi-platform installation (Codex prompts) (8h)
  - Action: Generate `.codex/prompts/*.md` files
  - Tests: 4 tests (generate prompt, multiple prompts, directory creation, permissions)

- [ ] **Story 3.6**: Implement conflict management (overwrite/skip/backup) (4h)
  - Action: Detect existing files, prompt user, execute choice
  - Tests: 6 tests (overwrite, skip, backup+overwrite, cancel, dry-run)
...
- [ ] **Story 5.1**: Implement error diagnostics (20h)
  - File: `install/lib/yanstaller/troubleshooter.js`
  - Action: Diagnose 6 error types (Node version, permissions, paths, platforms, YAML, MCP config)
  - Tests: 12 tests (each error type × diagnosis correct, suggestions helpful)

- [ ] **Story 5.2**: Implement auto-fix capabilities (12h)
  - Action: Auto-fix when possible (inject YAML frontmatter, repair MCP config, fix permissions)
  - Tests: 8 tests (each auto-fix × dry-run, apply, success, fail)
...
- `inquirer@^8.2.5` : Interactive CLI prompts (interview, custom selection)
- `fs-extra@^11.2.0` : Enhanced fs with promises (file operations, copy, backup)
- `chalk@^4.1.2` : Terminal colors (logger utility wrapper)
- `ora@^5.4.1` : Elegant spinners (progress tracking, installation feedback)
- `js-yaml@^4.1.0` : YAML parser (frontmatter extraction, config loading)
- `commander@^11.1.0` : CLI framework (arg parsing, subcommands)
...
- **Multi-OS** : Test Windows-specific issues (paths, permissions) on Windows VM if available
...
**Risk 1: Timeline Optimism (60% probability, HIGH impact)** :
- **Issue** : 248h estimation may be optimistic, especially Phase 3 Installer (56h most complex)
- **Mitigation** : Add 20% buffer (50h), prioritize Phase 1-3 (core functionality), Phase 4-8 can slip to v1.1
