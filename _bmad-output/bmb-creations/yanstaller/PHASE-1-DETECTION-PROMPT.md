# YANSTALLER - Phase 1: Detection - Prompt Quick-Flow-Solo-Dev

**Generated by**: BYAN-TEST  
**Date**: 2026-02-03  
**Phase**: 1 - Detection  
**Duration**: 40h (jours 3-7)  
**Module**: lib/yanstaller/detector.js + utilities

---

## 🎯 MISSION

Implémenter le module **DETECTOR** de YANSTALLER qui détecte automatiquement l'environnement utilisateur :
- OS (Windows/Linux/macOS)
- Node.js version
- Git présence
- 4 plateformes (Copilot CLI, VSCode, Claude Code, Codex)

---

## 📐 ARCHITECTURE CONTEXT

**Architecture**: `/home/yan/conception/_byan-output/bmb-creations/yanstaller/ARCHITECTURE.md`  
**Score validation**: 96/100 ✅  
**Status**: Production ready

**Design patterns appliqués**:
- Dependency Injection (pas de Singleton)
- Strategy pattern léger (platforms)
- Custom errors (6 classes)
- Simple state object

**Mantras prioritaires**:
- #37 Ockham's Razor (simplicité)
- #4 Fail Fast (Node check immédiat)
- IA-1 Trust But Verify (validation à chaque étape)
- IA-24 Clean Code (JSDoc complet)

---

## 📂 FICHIERS À IMPLÉMENTER

### 1. Utilities (Foundation)

#### `lib/utils/os-detector.js` ⭐ **START HERE**
**Status**: Skeleton avec JSDoc  
**Complexity**: LOW  
**Time**: 2h  

**Functions à implémenter**:
```javascript
detect() → {name, version, platform}
isWindows() → boolean
isMacOS() → boolean
isLinux() → boolean
```

**Tests requis**:
- `__tests__/utils/os-detector.test.js`
- 4 tests (detect, isWindows, isMacOS, isLinux)
- Mock `os.platform()` et `os.release()`

---

#### `lib/utils/node-detector.js`
**Status**: Skeleton avec JSDoc  
**Complexity**: LOW  
**Time**: 3h  

**Functions à implémenter**:
```javascript
detect() → string (e.g., '18.19.0')
compareVersions(v1, v2) → -1 | 0 | 1
meetsRequirement(current, required) → boolean
```

**⚠️ ATTENTION**: Gérer versions avec suffixes (`18.0.0-beta`)

**Tests requis**:
- `__tests__/utils/node-detector.test.js`
- 6 tests minimum
  - detect() retourne version string
  - compareVersions() cas: égal, supérieur, inférieur
  - meetsRequirement() cas: OK, trop vieux, suffixes

**Recommandation**: Utiliser regex pour strip suffixes avant comparaison

---

#### `lib/utils/git-detector.js`
**Status**: Skeleton avec JSDoc  
**Complexity**: LOW  
**Time**: 2h  

**Function à implémenter**:
```javascript
detect() → Promise<{installed: boolean, version: string | null}>
```

**Technique**: `execSync('git --version')` avec try/catch

**Tests requis**:
- `__tests__/utils/git-detector.test.js`
- 2 tests
  - Git installé → {installed: true, version: '2.x.x'}
  - Git absent → {installed: false, version: null}
- Mock `execSync`

---

### 2. Platform Detectors

#### `lib/platforms/copilot-cli.js`
**Status**: Partial skeleton  
**Complexity**: MEDIUM  
**Time**: 5h  

**Function à implémenter**:
```javascript
detect() → Promise<boolean>
```

**Logique**:
1. Check command `github-copilot-cli` existe
2. OU check directory `.github/agents/` existe

**Tests requis**:
- `__tests__/platforms/copilot-cli.test.js`
- 3 tests
  - CLI installé → true
  - Seulement .github/agents/ → true
  - Rien → false

---

#### `lib/platforms/vscode.js`
**Status**: Minimal skeleton (reuse copilot)  
**Complexity**: LOW  
**Time**: 2h  

**Note**: Réutilise `copilot-cli.detect()` (même format)

**Tests requis**:
- `__tests__/platforms/vscode.test.js`
- 1 test (appelle copilot-cli.detect())

---

#### `lib/platforms/claude-code.js`
**Status**: Partial skeleton  
**Complexity**: MEDIUM  
**Time**: 4h  

**Function à implémenter**:
```javascript
detect() → Promise<boolean>
```

**Logique**:
- Check config file existe selon OS:
  - macOS: `~/Library/Application Support/Claude/claude_desktop_config.json`
  - Windows: `~/AppData/Roaming/Claude/claude_desktop_config.json`
  - Linux: `~/.config/Claude/claude_desktop_config.json`

**Tests requis**:
- `__tests__/platforms/claude-code.test.js`
- 3 tests (un par OS)
- Mock `fs.pathExists()`

---

#### `lib/platforms/codex.js`
**Status**: Partial skeleton  
**Complexity**: LOW  
**Time**: 2h  

**Function à implémenter**:
```javascript
detect() → Promise<boolean>
```

**Logique**: Check `.codex/prompts/` directory exists

**Tests requis**:
- `__tests__/platforms/codex.test.js`
- 2 tests (exists / not exists)

---

### 3. Main Detector Module

#### `lib/yanstaller/detector.js` ⭐ **CRITICAL**
**Status**: Skeleton avec TODOs  
**Complexity**: HIGH  
**Time**: 10h  

**Functions à implémenter**:
```javascript
detect() → Promise<DetectionResult>
isNodeVersionValid(current, required) → boolean
detectPlatform(platformName) → Promise<PlatformInfo>
```

**Logique `detect()`**:
```javascript
async function detect() {
  // Parallel detection for speed
  const [osInfo, nodeVersion, gitInfo, platformsInfo] = await Promise.all([
    osDetector.detect(),
    nodeDetector.detect(),
    gitDetector.detect(),
    detectAllPlatforms()
  ]);
  
  return {
    os: osInfo.name,
    osVersion: osInfo.version,
    nodeVersion,
    hasGit: gitInfo.installed,
    gitVersion: gitInfo.version,
    platforms: platformsInfo
  };
}

async function detectAllPlatforms() {
  const platformNames = ['copilot-cli', 'vscode', 'claude', 'codex'];
  const results = await Promise.all(
    platformNames.map(name => detectPlatform(name))
  );
  return results;
}
```

**⚠️ AMÉLIORATION**: Ajouter timeout 10s par platform (recommandation BYAN-TEST)

**Tests requis**:
- `__tests__/yanstaller/detector.test.js`
- 10 tests minimum
  - detect() retourne structure complète
  - detect() parallel (Promise.all)
  - isNodeVersionValid() cas edge
  - detectPlatform() pour chaque platform
  - Timeout platform detection (si implémenté)

---

## 📊 ACCEPTANCE CRITERIA (Phase 1)

### Must Pass (Bloquants):
- [ ] AC-YAN-DET-01: Detect OS correctement (Win/Linux/macOS)
- [ ] AC-YAN-DET-02: Detect Node.js version (≥18.0.0 required)
- [ ] AC-YAN-DET-03: Detect Git presence (warning only si absent)
- [ ] AC-YAN-DET-04: Detect 4 platforms (Copilot CLI, VSCode, Claude, Codex)
- [ ] AC-YAN-DET-05: Generate detection report complet

### Quality (Non-bloquants mais fortement recommandés):
- [ ] Tests coverage > 80%
- [ ] JSDoc à jour (si modifs)
- [ ] Multi-OS tests pass (CI/CD)
- [ ] Pas d'emojis dans code (Mantra IA-23)
- [ ] Code self-documenting (Mantra IA-24)

---

## 🧪 TDD APPROACH (MANDATORY)

**Méthodologie**:
1. **Écrire tests AVANT code** (TDD strict)
2. Tests unitaires d'abord (utilities)
3. Tests d'intégration ensuite (detector.js)
4. Run tests après chaque fonction
5. Refactor si nécessaire

**Ordre recommandé**:
```
1. os-detector.test.js + os-detector.js
2. node-detector.test.js + node-detector.js
3. git-detector.test.js + git-detector.js
4. copilot-cli.test.js + copilot-cli.detect()
5. vscode.test.js + vscode.detect()
6. claude-code.test.js + claude-code.detect()
7. codex.test.js + codex.detect()
8. detector.test.js + detector.js (orchestration)
```

---

## 🚦 TESTS CI/CD

**Commandes**:
```bash
# Run tests
cd install
npm test

# Run tests avec coverage
npm test -- --coverage

# Run linter
npm run lint

# Fix linter auto
npm run lint:fix
```

**CI/CD déclenchement**:
- Push sur branch → GitHub Actions run automatiquement
- Matrix: 3 OS × 3 Node versions = 9 combinaisons
- Coverage upload vers Codecov (Ubuntu + Node 20 seulement)

---

## 🔧 DEPENDENCIES DISPONIBLES

**Déjà installées** (voir package.json):
- `inquirer` (CLI prompts) - pas nécessaire Phase 1
- `fs-extra` (file ops) - **OUI** pour fileUtils
- `chalk` (colors) - **OUI** pour logger
- `ora` (spinners) - pas nécessaire Phase 1
- `js-yaml` (YAML) - pas nécessaire Phase 1
- `commander` (CLI args) - pas nécessaire Phase 1

**DevDependencies**:
- `jest` - **OUI** pour tests
- `eslint` - **OUI** pour linting
- `prettier` - **OUI** pour formatting

**⚠️ SI BESOIN NOUVELLE DEP**: Justifier dans issue/PR

---

## 📝 CONTRAINTES TECHNIQUES

### Multi-OS
**CRITIQUE**: Code doit fonctionner sur 3 OS

**Bonnes pratiques**:
```javascript
// ✅ BON
const agentPath = path.join(projectRoot, '_byan', 'agents');

// ❌ MAUVAIS
const agentPath = projectRoot + '/_byan/agents'; // Fail sur Windows
```

### Error Handling
**Utiliser custom errors** (déjà créées dans `lib/errors.js`):
```javascript
const { NodeVersionError, PlatformNotFoundError } = require('../errors');

if (nodeVersion < '18.0.0') {
  throw new NodeVersionError('18.0.0', nodeVersion);
}
```

### Exit Codes
**Utiliser exit-codes.js** (nouvellement créé):
```javascript
const exitCodes = require('../exit-codes');

if (detectFailed) {
  process.exit(exitCodes.PLATFORM_ERROR);
}
```

---

## 🎯 MANTRAS À RESPECTER

### #37 - Ockham's Razor
**Application**: Code simple > code clever
- Pas de one-liners complexes
- Fonctions courtes (< 30 lignes)
- Nommage explicite

### #4 - Fail Fast
**Application**: Erreurs détectées immédiatement
- Node version check en premier
- Throw errors rapidement
- Pas de silent failures

### IA-1 - Trust But Verify
**Application**: Valider chaque détection
- Tests pour chaque function
- Edge cases couverts
- Multi-OS validation (CI)

### IA-24 - Clean Code
**Application**: Code self-documenting
- JSDoc complet si nouvelles functions
- Nommage clair (pas de `x`, `tmp`, `data`)
- Minimal comments (only WHY, not WHAT)

### IA-23 - No Emoji Pollution
**Application**: Zero emojis dans code
- ❌ Pas d'emojis dans JSDoc
- ❌ Pas d'emojis dans strings code
- ✅ Emojis OK dans CLI output (logger)

---

## 🐛 DEBUG TIPS

### Si tests échouent:
```bash
# Run un seul test file
npm test -- detector.test.js

# Run avec verbose
npm test -- --verbose

# Run avec watch mode
npm test -- --watch
```

### Si linter échoue:
```bash
# Voir erreurs
npm run lint

# Auto-fix
npm run lint:fix
```

### Si CI échoue:
1. Check logs GitHub Actions (`.github/workflows/yanstaller-test.yml`)
2. Reproduire localement même OS/Node version
3. Check multi-OS issues (path separator, permissions)

---

## 📚 RESSOURCES DISPONIBLES

**Architecture complète**:
- `_byan-output/bmb-creations/yanstaller/ARCHITECTURE.md` (27 KB)
- `_byan-output/bmb-creations/yanstaller/ARCHITECTURE-SUMMARY.md` (13 KB)

**Plan développement**:
- `_byan-output/bmb-creations/yanstaller/PLAN-DEVELOPPEMENT.md` (29 KB)

**Risques & mitigation**:
- `_byan-output/bmb-creations/yanstaller/RISKS.md` (8.3 KB)

**Specs agent**:
- `_byan-output/bmb-creations/yanstaller/AgentSpec-yanstaller.yaml` (9.7 KB)

**Code skeleton**:
- `install/lib/yanstaller/detector.js` (JSDoc + TODOs)
- `install/lib/utils/*.js` (7 utilities)
- `install/lib/platforms/*.js` (4 platforms + index)

---

## 🎊 DEFINITION OF DONE (Phase 1)

**Considéré terminé quand**:
- [x] Tous les AC-YAN-DET-01 à 05 passent
- [x] Tests coverage ≥ 80% (detector + utilities + platforms)
- [x] CI/CD passe sur 3 OS (GitHub Actions green)
- [x] Linter passe (0 errors)
- [x] Code review OK (BYAN-TEST validation)
- [x] Documentation à jour (JSDoc si modifs)
- [x] Zero emojis dans code (Mantra IA-23)
- [x] Commit messages clean (pas d'emojis)

---

## 🚀 COMMENCER MAINTENANT

**Première action**:
```bash
cd /home/yan/conception/install

# Créer branch
git checkout -b phase-1-detection

# Créer premier test
touch __tests__/utils/os-detector.test.js
```

**Premier test à écrire** (TDD):
```javascript
// __tests__/utils/os-detector.test.js
const osDetector = require('../../lib/utils/os-detector');

describe('osDetector', () => {
  describe('detect()', () => {
    it('should return OS information', () => {
      const result = osDetector.detect();
      
      expect(result).toHaveProperty('name');
      expect(result).toHaveProperty('version');
      expect(result).toHaveProperty('platform');
      
      expect(['windows', 'linux', 'macos']).toContain(result.name);
    });
  });
});
```

**Run test** (doit FAIL d'abord - TDD):
```bash
npm test -- os-detector.test.js
```

**Puis implémenter** `lib/utils/os-detector.js` pour faire passer test.

---

## 💡 CONSEILS QUICK-FLOW-SOLO-DEV

1. **TDD strict**: Tests avant code (toujours)
2. **Commits fréquents**: Après chaque function qui passe tests
3. **Push souvent**: CI détecte problèmes multi-OS tôt
4. **Consulte architecture**: Si doute sur design
5. **Challenge décisions**: Si quelque chose semble over-engineered
6. **Code simple**: Mantra #37 Ockham's Razor
7. **JSDoc à jour**: Si tu ajoutes functions

---

## 📞 SUPPORT

**Si besoin aide**:
- Architecture: Relis ARCHITECTURE.md
- Mantras: `@byan-test MAN` (Display 64 Mantras)
- Challenge design: Ping BYAN-TEST ou Winston
- Bugs multi-OS: Check CI logs → reproduire localement

---

**BON COURAGE ! 40h de dev, mais architecture solide = guidage clair.** 🚀

**TDD from Day 1. Tests before code. Always.** ✅
