...
| Piece | File | Role |
|-------|------|------|
| Codex pool | `providers/codex-provider.js` | wraps the `codex exec --json` system CLI (not an npm SDK); degrades to disabled if the binary is absent; two auth pools (CODEX_API_KEY per-token, else ChatGPT-subscription `~/.codex/auth.json`) |
